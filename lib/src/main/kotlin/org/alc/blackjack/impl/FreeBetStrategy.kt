...
class FreeBetStrategy(account: Account, gainFactor: Double? = null) : DefaultStrategy(account, gainFactor) {
    ...
    override fun hardDecision(hand: Hand, dealerCard: Card, table: Table) =
        if (hand.isFree) hardDecisionFreeHand(hand, dealerCard) else hardDecisionPayingHand(hand, dealerCard, table)
    ...
    private fun doubleIfEnoughFund(hand: Hand, fallbackDecision: Decision) =
        if (hand.initialBet <= account.balance()) Decision.DOUBLE else fallbackDecision

    private fun softDecisionFreeHand(hand: Hand, dealerCard: Card) = when (hand.score()) {
        in 12..15 -> Decision.HIT
        16 -> if (dealerCard.value == 6) doubleIfEnoughFund(hand, Decision.HIT) else Decision.HIT
        17 -> if (dealerCard.value in 5..6) doubleIfEnoughFund(hand, Decision.HIT) else Decision.HIT
        18 -> when (dealerCard.value) {
            in 4..6 -> doubleIfEnoughFund(hand, Decision.STAND)
            7 -> Decision.STAND
            else -> Decision.DOUBLE
        }
        19 -> if (dealerCard.value in 5..6) doubleIfEnoughFund(hand, Decision.STAND) else Decision.STAND
        20 -> if (dealerCard.value == 6) doubleIfEnoughFund(hand, Decision.STAND) else Decision.STAND
        21 -> Decision.STAND
        else -> throw IllegalStateException("Unexpected hand :${hand}")
    }
    ...
    private fun softDecisionPayingHand(hand: Hand, dealerCard: Card) = when (hand.score()) {
        in 12..15 -> Decision.HIT
        16 -> if (dealerCard.value == 6) doubleIfEnoughFund(hand, Decision.HIT) else Decision.HIT
        17 -> if (dealerCard.value in 5..6) doubleIfEnoughFund(hand, Decision.STAND) else Decision.HIT
        18 -> when (dealerCard.value) {
            in 2..4 -> Decision.STAND
            in 5..6 -> doubleIfEnoughFund(hand, Decision.STAND)
